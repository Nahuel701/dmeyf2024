...
AgregarVariables_IntraMes <- function(dataset) {
  ...
  if(atributos_presentes(c("cproductos", "ccuenta_corriente", "ccaja_seguridad", "ccaja_ahorro"))) {
    dataset[, ccuentas_agg := cproductos + ccuenta_corriente + ccaja_seguridad + ccaja_ahorro]
  }

  if(atributos_presentes(c("ctarjeta_debito_transacciones", "ctarjeta_visa_transacciones", "ctarjeta_visa_debitos_automaticos"))) {
    dataset[, ctrxs_tarjetas_visa := ctarjeta_debito_transacciones + ctarjeta_visa_transacciones + ctarjeta_visa_debitos_automaticos]
  }

  if(atributos_presentes(c("cpagomiscuentas", "ctransferencias_emitidas", "ccuenta_debitos_automaticos"))) {
    dataset[, c_transfers := cpagomiscuentas + ctransferencias_emitidas + ccuenta_debitos_automaticos]
  }

  if(atributos_presentes(c("ctarjeta_debito", "ctarjeta_visa", "ctarjeta_master"))) {
    dataset[, ctarjetas := ctarjeta_debito + ctarjeta_visa + ctarjeta_master]
  }

  if(atributos_presentes(c("ccuentras_agg", "ctrxs_trajetas_visa","c_transfers","ctarjetas"))) {
    dataset[, c_cants_aggs := ccuentas_agg + ctrxs_tarjetas_visa + c_transfers + ctarjetas] 
  }

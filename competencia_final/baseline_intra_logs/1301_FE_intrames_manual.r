...
AgregarVariables_IntraMes <- function(dataset) {
  ...
  json_data <- fromJSON("~/buckets/b1/references/columns_to_log.json")

  # Loop through each variable in the JSON data
  for (i in 1:nrow(json_data)) {
    var <- json_data$Variable[i]
    ...
    if (var %in% colnames(dataset)) {
      ...
      # Perform the logarithm operation and add the result as a new column to the dataset
      dataset[, (new_col) := log(get(var))]
    }
  }

...
AgregarVariables_IntraMes <- function(dataset) {
  ...
  if(atributos_presentes(c("ccomisiones_mantenimiento", "ccomisiones_otras"))) {
    dataset[, ccomisiones_mantenimiento_suma_ccomisiones_otras := ccomisiones_mantenimiento + ccomisiones_otras]
  }
  if(atributos_presentes(c("mrentabilidad", "mrentabilidad_annual"))) {
    dataset[, mrentabilidad_suma_mrentabilidad_annual := mrentabilidad + mrentabilidad_annual]
  }

  # Payroll
  if(atributos_presentes(c("cpayroll_trx", "cpayroll2_trx"))) {
    dataset[, cpayroll_trx_suma_cpayroll2_trx := cpayroll_trx + cpayroll2_trx]
  }
  if(atributos_presentes(c("mpayroll", "mpayroll2"))) {
    dataset[, mpayroll_suma_mpayroll2 := mpayroll + mpayroll2]
  }

  # Cuentas y ahorros
  if(atributos_presentes(c("ccuenta_corriente", "ccaja_ahorro"))) {
    dataset[, ccuenta_corriente_suma_ccaja_ahorro := ccuenta_corriente + ccaja_ahorro]
  }
  if(atributos_presentes(c("mcuenta_corriente", "mcaja_ahorro"))) {
    dataset[, mcuenta_corriente_suma_mcaja_ahorro := mcuenta_corriente + mcaja_ahorro]
  }

  # Descubiertos y préstamos
  if(atributos_presentes(c("cdescubierto_preacordado", "cprestamos_personales"))) {
    dataset[, cdescubierto_preacordado_suma_cprestamos_personales := cdescubierto_preacordado + cprestamos_personales]
  }
  if(atributos_presentes(c("mprestamos_personales", "mprestamos_prendarios"))) {
    dataset[, mprestamos_personales_suma_mprestamos_prendarios := mprestamos_personales + mprestamos_prendarios]
  }

  # Plazo fijo e inversiones
  if(atributos_presentes(c("cplazo_fijo", "cinversion1"))) {
    dataset[, cplazo_fijo_suma_cinversion1 := cplazo_fijo + cinversion1]
  }
  if(atributos_presentes(c("mplazo_fijo_pesos", "minversion1_pesos"))) {
    dataset[, mplazo_fijo_pesos_suma_minversion1_pesos := mplazo_fijo_pesos + minversion1_pesos]
  }

  # Forex
  if(atributos_presentes(c("cforex_buy", "cforex_sell"))) {
    dataset[, cforex_buy_suma_cforex_sell := cforex_buy + cforex_sell]
  }
  if(atributos_presentes(c("mforex_buy", "mforex_sell"))) {
    dataset[, mforex_buy_suma_mforex_sell := mforex_buy + mforex_sell]
  }

  # Seguros
  if(atributos_presentes(c("cseguro_vida", "cseguro_auto"))) {
    dataset[, cseguro_vida_suma_cseguro_auto := cseguro_vida + cseguro_auto]
  }

  # Tarjetas de débito y crédito
  if(atributos_presentes(c("ctarjeta_debito", "ctarjeta_debito_transacciones"))) {
    dataset[, ctarjeta_debito_suma_ctarjeta_debito_transacciones := ctarjeta_debito + ctarjeta_debito_transacciones]
  }
  if(atributos_presentes(c("mautoservicio"))) {
    dataset[, mautoservicio := mautoservicio]
  }

  # Débitos automáticos
  if(atributos_presentes(c("ccuenta_debitos_automaticos", "ctarjeta_visa_debitos_automaticos"))) {
    dataset[, ccuenta_debitos_automaticos_suma_ctarjeta_visa_debitos_automaticos := ccuenta_debitos_automaticos + ctarjeta_visa_debitos_automaticos]
  }
  if(atributos_presentes(c("mcuenta_debitos_automaticos", "mttarjeta_visa_debitos_automaticos"))) {
    dataset[, mcuenta_debitos_automaticos_suma_mttarjeta_visa_debitos_automaticos := mcuenta_debitos_automaticos + mttarjeta_visa_debitos_automaticos]
  }

  # Pagos de servicios y mis cuentas
  if(atributos_presentes(c("cpagodeservicios", "cpagomiscuentas"))) {
    dataset[, cpagodeservicios_suma_cpagomiscuentas := cpagodeservicios + cpagomiscuentas]
  }
  if(atributos_presentes(c("mpagodeservicios", "mpagomiscuentas"))) {
    dataset[, mpagodeservicios_suma_mpagomiscuentas := mpagodeservicios + mpagomiscuentas]
  }

  # Descuentos
  if(atributos_presentes(c("ccajeros_propios_descuentos", "ctarjeta_visa_descuentos"))) {
    dataset[, ccajeros_propios_descuentos_suma_ctarjeta_visa_descuentos := ccajeros_propios_descuentos + ctarjeta_visa_descuentos]
  }
  if(atributos_presentes(c("mcajeros_propios_descuentos", "mtarjeta_visa_descuentos"))) {
    dataset[, mcajeros_propios_descuentos_suma_mtarjeta_visa_descuentos := mcajeros_propios_descuentos + mtarjeta_visa_descuentos]
  }

  # Transferencias y cheques
  if(atributos_presentes(c("ccheques_depositados", "ccheques_emitidos"))) {
    dataset[, ccheques_depositados_suma_ccheques_emitidos := ccheques_depositados + ccheques_emitidos]
  }
  if(atributos_presentes(c("mcheques_depositados", "mcheques_emitidos"))) {
    dataset[, mcheques_depositados_suma_mcheques_emitidos := mcheques_depositados + mcheques_emitidos]
  }

  # Canales de atención
  if(atributos_presentes(c("ccajas_transacciones", "ccajas_depositos"))) {
    dataset[, ccajas_transacciones_suma_ccajas_depositos := ccajas_transacciones + ccajas_depositos]
  }
  if(atributos_presentes(c("matm", "matm_other"))) {
    dataset[, matm_suma_matm_other := matm + matm_other]
  }
  if(atributos_presentes(c("thomebanking", "cmobile_app_trx"))) {
    dataset[, thomebanking_suma_cmobile_app_trx := thomebanking + cmobile_app_trx]
  }
  ...
  for (i in 1:nrow(json_data)) {
    var1 <- json_data$Variable1[i]
    var2 <- json_data$Variable2[i]
    
    # Check if the variables exist in the dataset
    if (all(c(var1, var2) %in% colnames(dataset))) {
      
      # Generate new column name for the result
      new_col <- paste0(var1,"_",operation, "_", var2)

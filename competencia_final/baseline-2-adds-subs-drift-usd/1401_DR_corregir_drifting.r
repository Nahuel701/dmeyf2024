...
dolar_tarjeta <- c(
   ...
   44.354211,  46.088636,  44.955000,
   43.751429,  54.650476,  58.790000,
   61.403182,  63.012632,82.0,82.0,83.0,
   85.0,87.0,91.0,94.0,97.0,100.0,
   117.0,136.0,140.0,145.0,150.0
   ,154.0,159.0,162.0,164.0,165.0,
   167.0,169.0,171.0
)
...
# Identificar los nombres de las columnas que comienzan con 'm', 'Visa_m', o 'Master_m'
campos_monetarios <- colnames(dataset)[grepl("^(m|Visa_m|Master_m)", colnames(dataset))]

# Identificar columnas relacionadas con transacciones en dólares (txs_usd)
campos_txs_usd <- colnames(dataset)[grepl("^m.*dolares", colnames(dataset))]

# Identificar columnas de tarjetas relacionadas con dólares (tarjeta_usd)
campos_tarjeta_usd <- colnames(dataset)[grepl("^(Visa_m|Master_m).*dolares", colnames(dataset))]

# Remover campos de transacciones en dólares y campos de tarjetas en dólares de los campos monetarios
campos_monetarios <- setdiff(campos_monetarios, c(campos_txs_usd, campos_tarjeta_usd))

# Imprimir resultados para verificar
cat("Campos monetarios:", campos_monetarios, "\n")
cat("Campos de transacciones USD:", campos_txs_usd, "\n")
cat("Campos de tarjetas USD:", campos_tarjeta_usd, "\n")
